...
We can see that in this code the text entered is run through the eval() function, and before getting into eval the following characters are removed from the string: ['$', '(', ')', '`', '"', "'", "+", ":", "/", "!", "?"] and "()" is added to the end, so we will not be able to call any functions with arguments

## Attack

After trying out all possible options, a solution was found:
```PHP
@include DIRECTORY_SEPARATOR.flag;#
```
after that we get a flag


## Results

I didn't save the flag...
